...
def _get_answer_text(resp: dict) -> str:
    """
    LangChainの戻り値が answer / output_text どちらでも落ちないように吸収
    """
...
def _normalize_sub_choices(sub_choices):
    """
    sub_choices が list[dict] / list[str] どちらでも表示できるように正規化
    """
    out = []
    if not sub_choices:
        return out
...
def display_select_mode():
    ...
    with col1:
        # label空は警告が出るためダミー文字を入れる（Cloudログ対策）
...
# ==========================================================
# ★ main.py互換用（重要）
# main.py が display_contact_llm_response を呼ぶため、
# ここで必ず提供する（パターンA）
# ==========================================================
def display_contact_llm_response(llm_response):
    return display_inquiry_llm_response(llm_response)